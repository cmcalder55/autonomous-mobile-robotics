...
function [path, path_length] = RRT(start_state, goal_region, obstacles)
    ...
    % while robot is not within goal region, pick a random state within 
    % the workspace and find closest neighbors to populate path tree
    while tree(:,1) < goal_region(1)  
        
        rand_state = rand(1,2)*100;   % random state within workspace
        [neighbors,tree] = build_RRT(rand_state,tree,obstacles,epsilon,neighbors);  
    
    end
    
    % find indices of nodes in tree matching the neighbor of the robots final pose
    path = tree(end,:);                               % initialize with final element
    match = abs(neighbors(end,:) - tree);   
    n = find(match < tol);                      % find where difference between elements is 0
